...
bool find(const vector<int> &v, int target); //vec of int by reference

int main() {  //KISS 
    
    vector<int> numbers;        // our list of numbers
    char selection {};//char VAR
    
    do {
        display_menu(); //Fn to disp menu being called
        selection = get_selection();  // below design allows us to think modularly
        switch (selection) { //handle_ = all Fn
            case 'P': 
                handle_display(numbers); //pass vec = numbers by ref and const
                break;
            case 'A': 
                handle_add(numbers);
                break;
...
***************************************************************/
char get_selection() {      //does return a char
    char selection {};
    cin >> selection;  //simply reading from keyboad 
    return toupper(selection);  //conv to CAPS
}
...
***************************************************************/
void handle_display(const vector<int> &v) { //pass by ref and const
...
void handle_find(const vector<int> &v) { 
    ...
    cin >> target;
    if ( find(v, target))  //returns true then int is in there
...
int get_smallest(const vector<int> &v) {
    int smallest = v.at(0);
    for (auto num: v)  //loop each element 
        if (num < smallest)
            smallest = num;
    return smallest;
}
...
this function should NOT modify the list of numbers
***************************************************************/
bool find(const vector<int> &v, int target) { //compare each element to target
    for (auto num: v)  //loop each element 
        if (num == target)
            return true;
    return false;
